...
from tensorflow.keras.layers import Input, Conv2D, MaxPooling2D, Dense, Flatten, Dropout, BatchNormalization, Activation, concatenate, AveragePooling2D, GlobalAveragePooling2D, Add, Lambda
...
def Inception_ResNet_A(x, scale=0.1):   
  branch_1 = Conv2D(filters = 32, kernel_size = (1, 1), padding='same', strides=1)(x)
     
  branch_2 = Conv2D(filters = 32, kernel_size = (1, 1), padding='same', strides=1)(x)
  branch_2 = Conv2D(filters = 32, kernel_size = (3, 3), padding='same', strides=1)(branch_2)

  branch_3 = Conv2D(filters = 32, kernel_size = (1, 1), padding='same', strides=1)(x)
  branch_3 = Conv2D(filters = 32, kernel_size = (3, 3), padding='same', strides=1)(branch_3)
  branch_3 = Conv2D(filters = 32, kernel_size = (3, 3), padding='same', strides=1)(branch_3)
        
  branches = concatenate([branch_1, branch_2, branch_3], axis = 3)
  Inception = Conv2D(filters = 256, kernel_size = (1, 1), padding='same', strides=1, activation = None)(branches)

  scaled_activation = Scaling_Residual(Inception, scale=scale)
    
  residual_connection = Add()([x, scaled_activation])

  return residual_connection

def Inception_ResNet_B(x, scale=0.1):
  branch_1 = Conv2D(filters = 128, kernel_size = (1, 1), padding='same', strides=1)(x)
        
  branch_2 = Conv2D(filters = 128, kernel_size = (1, 1), padding='same', strides=1)(x)
  branch_2 = Conv2D(filters = 128, kernel_size = (1, 7), padding='same', strides=1)(branch_2)
  branch_2 = Conv2D(filters = 128, kernel_size = (7, 1), padding='same', strides=1)(branch_2)
        
  branches = concatenate([branch_1, branch_2])
  Inception = Conv2D(filters = 896, kernel_size = (1, 1), padding='same', strides=1, activation = None)(branches)
...
def Inception_ResNet_C(x, scale=0.1):    
  branch_1 = Conv2D(filters = 192, kernel_size = (1, 1), padding='same', strides=1)(x)
        
  branch_2 = Conv2D(filters = 192, kernel_size = (1, 1), padding='same', strides=1)(x)
  branch_2 = Conv2D(filters = 192, kernel_size = (1, 3), padding='same', strides=1)(branch_2)
  branch_2 = Conv2D(filters = 192, kernel_size = (3, 1), padding='same', strides=1)(branch_2)      
        
  branches = concatenate([branch_1, branch_2])
  Inception = Conv2D(filters = 1792, kernel_size = (1, 1), padding='same', strides=1, activation = None)(branches)
...
def Reduction_B(x):
  ...
  branch_2 = Conv2D(filters = 384, kernel_size = (3, 3), padding='valid', strides=2)(branch_2)

  branch_3 = Conv2D(filters = 256, kernel_size = (1, 1), padding='same', strides=1)(x)
  branch_3 = Conv2D(filters = 256, kernel_size = (3, 3), padding='valid', strides=2)(branch_3)
  
  
  branch_4 = Conv2D(filters = 256, kernel_size = (1, 1), padding='same', strides=1)(x)
  branch_4 = Conv2D(filters = 256, kernel_size = (3, 3), padding='same', strides=1)(branch_4)
  branch_4 = Conv2D(filters = 256, kernel_size = (3, 3), padding='valid', strides=2)(branch_4)
...
# 두 개의 입력층을 정의
    ...
    x = MaxPooling2D((3, 3), padding='valid', strides=2)(x) # 147x147x64 -> 73x73x64
        
    x = Conv2D(filters = 80, kernel_size = (1, 1), padding='same', strides=1)(x) # 73x73x64 -> 73x73x80
    x = Conv2D(filters = 192, kernel_size = (3, 3), padding='valid', strides=1)(x) # 73x73x80 -> 71x71x192U
    x = Conv2D(filters = 256, kernel_size = (3, 3), padding='valid', strides=2)(x) # 73x73x80 -> 71x71x192U
        


    for i in range(5):
      x = Inception_ResNet_A(x, scale=0.17)    
    x = Reduction_A(x)
    for i in range(10):
      x = Inception_ResNet_B(x, scale=0.1)
    x = Reduction_B(x)
    for i in range(5):
      x = Inception_ResNet_C(x, scale=0.2)
    ...
    model.save('/gdrive/My Drive/Colab/대개론_CNN/googlenet_res_v1_epochs150_not_rescale.model')
